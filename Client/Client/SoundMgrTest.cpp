#include "stdafx.h"
#include "SoundMgrTest.h"
IMPLEMENT_SINGLETON(CSoundMgrTest)

CSoundMgrTest::CSoundMgrTest()
	:m_pSystem(nullptr)
{
	Initialize();
}
...
void CSoundMgrTest::Update()
{
}

void CSoundMgrTest::Release()
{
...
void CSoundMgrTest::PlaySound(TCHAR * pSoundKey, CHANNELID eID)
{
...
void CSoundMgrTest::PlayBGM(TCHAR * pSoundKey)
{
	if (m_mapSound.empty())
		return;
	map<TCHAR*, FMOD_SOUND*>::iterator iter = find_if(m_mapSound.begin(), m_mapSound.end(), CMyStrCmp(pSoundKey));
...
void CSoundMgrTest::StopSound(CHANNELID eID)
{
	FMOD_Channel_Stop(m_pChannel[eID]);
}

void CSoundMgrTest::StopAll()
{
...
void CSoundMgrTest::LoadSoundFile()
{
	_finddata_t fd;
